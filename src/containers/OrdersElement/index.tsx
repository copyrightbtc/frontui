        ...
        return (
            <thead>
                <tr>
                            ...
                            <th>
                                <OverlayTrigger 
                                    ...
                                    overlay={<Tooltip title="page.body.openOrders.header.avgPrice.tooltip.description" />}>
                                        <div className="dotted">{this.props.intl.formatMessage({ id: 'page.body.openOrders.header.avgPrice' })}</div>
                                </OverlayTrigger>
                            </th>
                    ...
                    <th>{this.props.intl.formatMessage({ id: 'page.body.openOrders.header.amountRemaining' })}</th>
                    <th>{this.props.intl.formatMessage({ id: 'page.body.openOrders.header.totalRemaining' })}</th>
        ...
        return [
            <tr>
                ...
                {this.props.type !== 'open' ?
                    <React.Fragment>
                        <td>
                            {orderType === 'Market' ? <div key={id} className="coins-name"><Decimal fixed={currentMarket.price_precision} thousSep=",">{avg_price}</Decimal><span>{curMarket?.quote_unit?.toUpperCase()}</span></div> : '---'}
                        </td>
                        <td>
                            {price && orderType !== 'Market' ? <div key={id} className="coins-name"><Decimal fixed={currentMarket.price_precision} thousSep=",">{price}</Decimal><span>{curMarket?.quote_unit?.toUpperCase()}</span></div> : '---'}
                        </td>
                    </React.Fragment> : 
                    <td>
                        <div key={id} className="coins-name"><Decimal fixed={currentMarket.price_precision} thousSep=",">{price}</Decimal><span>{curMarket?.quote_unit?.toUpperCase()}</span></div>
                    </td>
                }
                <td>
                    <div className="coins-name" key={id}><Decimal fixed={currentMarket.amount_precision} thousSep=",">{origin_volume}</Decimal><span>{curMarket?.base_unit?.toUpperCase()}</span></div>
                </td>
                <td>
                    <div className="coins-name" key={id}><Decimal fixed={currentMarket.price_precision} thousSep=",">{total}</Decimal><span>{curMarket?.quote_unit?.toUpperCase()}</span></div>
                </td>
                <td> 
                    <div className={`filled-order${+filled === 100 ? '' : ' full'}`} key={id}>
                        <Decimal fixed={2} thousSep=",">{+filled}</Decimal>%
                    </div>
                </td>
                <td>
                    <div className="coins-name" key={id}><Decimal fixed={currentMarket.amount_precision} thousSep=",">{remaining_volume}</Decimal><span>{curMarket?.base_unit?.toUpperCase()}</span></div>
                </td>
                <td>
                    <div className="coins-name" key={id}><Decimal fixed={currentMarket.price_precision} thousSep=",">{totalRemaining}</Decimal><span>{curMarket?.quote_unit?.toUpperCase()}</span></div>
                </td>
                <td>
